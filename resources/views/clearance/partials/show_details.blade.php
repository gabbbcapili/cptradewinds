...
<div class="modal-body" >
  <div class="row invoice-info">
    ...
    <div class="col-sm-4 invoice-col">
        ...
        Customer: <strong>{{ $clearance->fullname }}</strong>
     Email: {{ $clearance->email }} 
      Contact Number: {{ $clearance->mobile_number }}
      </div>
    </div>
...
@if($clearance->invoice)
  ...
  @endif
    @if($clearance->customer_deposit)
  <div class="row invoice-info no-print">
    <div class="col-sm-8 invoice-col">
      <div class="form-group pull-left">
        <label>Customer Deposit:</label>
        <a target="_blank" href="{{ $clearance->get_customer_deposit_url() }}">View Customer Deposit</a>
      </div>
    </div>
  </div>
  @endif
